    ...
    $('#view-comments-modal').on('show.bs.modal', function (event) {
        ...
        $.get("/api/article/get/all/comments/"+ articleId , (articleComments) => {
            ...
            articleComments.comments.forEach(comment => {
                let listComment = $("<li>")
                .html(`<p title="Click the trash icon to delete comment.">
                    ${comment.text} &nbsp;<button title="Delete comment" type="button"  data-commentid="${comment._id}" 
                    data-articleid="${articleId}"  class="delete-comment badge badge-danger"><i class="fas fa-trash-alt"></i></button></p>`
                );
